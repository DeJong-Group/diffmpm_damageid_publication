...
noisetype_choices = ['particle', 'uniform']
SNR_dB_choices = ['20', '10', '0']

obs = obs_choices[0]
case = case_choices[0]
noisetype = noisetype_choices[0]
SNR_dB = int(SNR_dB_choices[0])
np.random.seed(42)
n_blocks_x = 180
n_blocks_y = 12
...
target_strain_np = np.load(f'results/true/s_cs_{case}_f.npy')
...
# run naive once
obs_cases = [
        f"results_42/direct/{case}_{obs}_{noisetype}_{SNR_dB}.json",
    ]
...
def stable_regions(x, reference, reduction_percent = 50):
    deviation = np.abs(x - reference)
    max_deviation = np.max(deviation)
    exceed_indices = np.where(deviation > max_deviation * reduction_percent / 100)[0]
...
mask_start = 4e3
exceed = stable_regions(np.array(result), 4e3, reduction_percent=50)
if len(exceed) == 0:
...
result_2 = minimize(compute_loss_and_grad_modular,
                    ...
                    jac=True,
                    options=options,
                    bounds = [(0, 4000) for i in range(n_targets)]
                    )
print(result_2.x)

filename = f"results_42/mask/{case}_{obs}_start_{int(mask_start)}_{noisetype}_{SNR_dB}.json"
