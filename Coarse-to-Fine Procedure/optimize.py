...
deviation_threshold_choices = [0.3, 0.5, 0.8]
noisetype_choices = ['particle', 'uniform']
SNR_dB_choices = ['20', '10', '0']

obs = obs_choices[0]
case = case_choices[0]
deviation_threshold = deviation_threshold_choices[0]
...
for z in range(4):
    ...
    while deviation2 < converged_threshold:
        ...
        converge_counter += 1
        print(converge_counter, f"r_c_{case}_{obs}_{int(deviation_threshold*10)}_{noisetype}_{SNR_dB}, ", "block size: ", sub_block_order[z + 1])
...
filename = f"{case}_{obs}_{int(deviation_threshold*10)}_{noisetype}_{SNR_dB}.json"
