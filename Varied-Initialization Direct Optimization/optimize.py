...
damaged_choices = ["0", "1"]
init_e_choices = [2000, 4000, 6000]

# Extract arguments
obs = obs_choices[0]
damaged = damaged_choices[0]
init_e = init_e_choices[0]
...
if optim == 'lbfgs':
    ...
    result = minimize(compute_loss_and_grad,
                      ...
                      method='L-BFGS-B',
                      jac=True,
                      bounds = [(0, 4e3) for _ in initial_params],
                      options=options)
    t2 = time.time()
    ...
    with open(f"{int(damaged)}_{int(init_e)}_{obs}.json", "w") as outfile: 
